#!/usr/bin/env bash
# Rung 3 of the Phase 0 test ladder: class with constructor, typed property,
# and method call on $this.
set -euo pipefail
cd "$(dirname "$0")/../src"
...
echo "--- dump ---"
OPDUMP_MODE=dump OPDUMP_OUT="$BLOB" "$PHP_BIN" -n -d extension="$SO" -f ../tests/rung3.php
